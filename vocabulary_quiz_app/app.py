...
class VocabularyQuizApp:
    def __init__(self, root: tk.Tk, words: list[Word]) -> None:
        ...
        self.total = 0
        self.question_num = 0
        self.reverse = False  # False: 영→한, True: 한→영
        ...
        self.mode_button = ttk.Button(root, textvariable=self.mode_var, command=self.toggle_mode)
        # mode_var를 버튼 텍스트로 재활용하지 않고 별도 텍스트 사용
        self.mode_button = ttk.Button(root, text="⇄ 출제 방향 전환", command=self.toggle_mode)
        self.mode_button.pack(pady=(4, 0))
    ...
    def next_word(self) -> None:
        ...
        self.progress_var.set(f"{self.question_num}번째 문제 / 전체 {len(self.words)}개")
